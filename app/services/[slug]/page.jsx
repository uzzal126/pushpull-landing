...
export async function generateMetadata({ params }) {
  ...
  const openGraphImage = {
    images: [
      `https://cms.webmanza.com${service?.data[0]?.attributes?.iconBig?.data?.attributes?.url}`,
    ],
  };

  return {
    title: service?.data[0]?.attributes?.title,
    description: service?.data[0]?.attributes?.description,
    openGraph: {
      title: service?.data[0]?.attributes?.title,
      description: service?.data[0]?.attributes?.description,
      ...openGraphImage,
